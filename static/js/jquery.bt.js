...
jQuery.bt = {version: '0.9.5-rc1-dp'}; //Double patched http://plugins.jquery.com/node/11746 and http://plugins.jquery.com/node/8716
...
;(function($) { 
  ...
  jQuery.fn.bt = function(content, options) {
    ...
    return this.each(function(index) {
      ...
      this.btOn = function () {
        ...
        if (opts.attachTo) {
          var attachTo = $(opts.attachTo, this);
        }
        else {
          var attachTo = $(this);
        }       
        ...
        if (opts.offsetParent){
          ...
          var offsetParent = ($(attachTo).css('position') == 'absolute') ? $(attachTo).parents().eq(0).offsetParent() : $(attachTo).offsetParent();
          var pos = $(attachTo).btPosition();
          var top = numb(pos.top) + numb($(attachTo).css('margin-top')) - shadowShiftY; // IE can return 'auto' for margins
          var left = numb(pos.left) + numb($(attachTo).css('margin-left')) - shadowShiftX;
        }

        var width = $(attachTo).btOuterWidth();
        var height = $(attachTo).outerHeight();
  ...
  */
  ...
  jQuery.bt.defaults = {
    ...
    attachTo:          null,                 // Element within the trigger element to attach the tooltip to.
